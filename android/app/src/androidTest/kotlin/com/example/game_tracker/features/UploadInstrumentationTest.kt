...
import com.example.game_tracker.application.feature.UploadFeature
...
class UploadInstrumentationTest {

    @Test
    fun executeUploadFeatureOnDeviceRuntime() = runBlocking {
        val fakeUploadController = FakeUploadController()
        val uploadFeature = UploadFeature(fakeUploadController)

        val commandId = CommandId("cmd_up_${UUID.randomUUID()}")
        val traceId = TraceId("trace_up_${UUID.randomUUID()}")
        ...
        val command = UploadCommand(metadata, localFilePath = "/sdcard/test.png")
        ...
        val result = uploadFeature.execute(command, systemServices)
